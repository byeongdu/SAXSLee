 ...
 ***************************************************************************/
void get_pck(int x, int y, unsigned char *cimg, WORD *img)
{ 
unsigned char	*ci;
 ...
 *****************************************************************************/
static void 
unpack_word(unsigned char *cimg, int x, int y, WORD *img)
{ 
...
i = 0;
	...
	while (pixel < total) {
	    if (valids < 6) {
    		if (spillbits > 0) {
				//printf("spillbits %d, spill %d, valids %d, window %d\n", spillbits, spill, valids, window);
      			window |= shift_left(spill, valids);
				//printf("spill %d, valids %d, window %d\n", spill, valids, window);
        		valids += spillbits;
        		spillbits = 0;
		}
      	    	else {
    		...
    		pixnum = 1 << (window & setbits[3]);
      		...
      		while ((pixnum > 0) && (pixel < total)) {
      		    if (valids < bitnum) {
        		if (spillbits > 0) {
					//printf("AAA spillbits %d, spill %d, valids %d, window %d\n", spillbits, spill, valids, window);
          		    window |= shift_left(spill, valids);
					//printf("AAA spillbits %d, spill %d, valids %d, window %d\n", spillbits, spill, valids, window);
					...
					//printf("BBB spillbits %d, spill %d, valids %d, window %d\n", spillbits, spill, valids, window);
			        spillbits -= usedbits;
			        valids = 32;
			    }
			}
	...
	//printf("pixel %d, pixnum %d, window %d, nextint %d, spill %d, bitnum %d, usedbits %d\n", pixel, pixnum, window, nextint, spill, bitnum, usedbits);
	}
	//for (i=0;i<10000;i++)
	//	printf("%d\n", img[i]);
}
...
void mexFunction( int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[])
{
	...
	int sx, sy;						/* Side length of uncompressed image */
    ...
    plhs[0] = mxCreateNumericMatrix((mwSize)sx,(mwSize)sy,mxUINT16_CLASS,mxREAL);

    /* get a pointer to the real data in the output matrix (img) */
    outImage = mxGetData(plhs[0]);

    /* call the computational routine */
    get_pck(sx,sy,cimg,outImage);
}
/*#endif*/
