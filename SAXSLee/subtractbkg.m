...
% SUBTRACTBKG GUI to subtract constant background; called by SAXSLee.
% 
...
% --- check whether the current curve is merged scan. If yes, return;
for iLine = 1:length(hLine)
    if ~strcmp(lineTags{iLine}(1:9),'SAXSLeeScan');
        uiwait(msgbox('Background subtraction does not apply to merged scans!','Background Subtraction Error','error','modal'));
        return;
    end
end
