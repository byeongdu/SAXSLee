function SAXSLee_datalabel(state,marker_color);
%SAXSLee_datalabel       Interactive labeling of data points on a plot
%   SAXSLee_datalabel with no arguments toggles the data tip state
%   SAXSLee_datalabel ON turns data tips on for the current figure
%   SAXSLee_datalabel OFF turns data tips off for the current figure
%   SAXSLee_datalabel('ON',S), where S is a character string specifying
...
%            SAXSLee_datalabel('on','.b');  %Add blue dots with next data tips
%
...
if nargin == 0 | strcmp(state,'on') % Set the WindowButtonDownFcn
    ...
    set(hFigSAXSLee,'WindowButtonDownFcn','SAXSLee_datalabel down')
    set(hFigSAXSLee, 'KeyPressFcn', 'SAXSLee_datalabel keypress')    
    setappdata(hFigSAXSLee,'Marker',marker);
    setappdata(hFigSAXSLee,'Color',color);
    set(hFigSAXSLee,'DoubleBuffer','on');       %eliminate flicker
elseif strcmp(state, 'keypress')
    set(gcf, 'KeyPressFcn', @selectline);
  
elseif strcmp(state,'down') % Execute the WindowButtonDownFcn
    htype = get(gco,'Type');
    tag = get(gco,'Tag');
    marker = getappdata(hFigSAXSLee,'Marker');
    color  = getappdata(hFigSAXSLee,'Color');
    ...
    if strcmp(htype,'line') & ~strcmp(tag,'Dot')     %Line - Add text object
        ...
        [xv,yv, ind] = local_nearest(x,xl,y,yl);

        
        %For R13 or higher (MATLAB 6.5), use a background color on the text string
        th = text(xv,yv,['  (' num2str(xv) ', ' num2str(yv) ', ' num2str(ind) ')']);
    ...
    elseif strcmp(htype,'text') | strcmp(tag,'Dot')
        ...
        set(hFigSAXSLee,'WindowButtonMotionFcn','SAXSLee_datalabel move', ...
            'WindowButtonUpFcn','SAXSLee_datalabel up');
    end
...
elseif strcmp(state,'move') % Execute the WindowButtonMotionFcn
    ...
    if ~isempty(gco) & ~isempty(get(gco,'UserData'))    %Second check catches legend
        ...
        if slide            %Move to new data value
            ...
            set(th,'Position', [xv yv 0],'String',['  (' num2str(xv) ', ' num2str(yv) ', ' num2str(ind) ')'])
...
elseif strcmp(state,'up') % Execute the WindowButtonUpFcn
    ...
    if strcmp(htype,'text') | strcmp(tag,'Dot')
        ...
        set(hFigSAXSLee,'WindowButtonMotionFcn','')
    end;
elseif strcmp(state,'off') % Unset the WindowButton...Fcns
    set(hFigSAXSLee,'WindowButtonDownFcn','','WindowButtonUpFcn','')
    set(hFigSAXSLee, 'KeypressFcn', '');
end


function [xv,yv,ind]=local_nearest(x,xl,y,yl)
%Inputs:
% x   Selected x value
% xl  Line Data (x)
% y   Selected y value
...
%    ch = getappdata(src, 'currentline');
    ...
    %t = findobj(gca, 'type', 'line','Visible','on');
    %[hdl, indx, xdata, ydata, ydataError, tg, scanIndx] = SAXSLee_findcursor;
    keyN = str2double(sprintf('%d', evnt.Character));
    [hdln, indx, xd, yd, data, tg, scanIndx, dothdln, txthdln] = SAXSLee_findcursor;
  
    
    if (strcmp(evnt.Character, 'x'))
        for kk = 1:numel(hdln)
            hdl = hdln(kk);
            dothdl = dothdln(kk);
            txthdl = txthdln(kk);
            try
            if strfind(get(hdl, 'tag'), 'REF#')
                [T, RR] = strtok(get(hdl, 'Tag'), 'REF#');
                [T, RR] = strtok(T, ':');
                refscan{str2num(T)} = [];
            end
            delete(hdl);
        ...
        try
            settings.scan.selectionIndex(scanIndx) = [];
            settings.scan.selection{scanIndx} = [];
        catch
            
        end
    elseif (strcmp(evnt.Character, 'a'))
        kN = 0;
        for i=2:numel(refscan)
        ...
        refscan{kN}.colData = data;
        refscan{kN}.Tag = tg;
        refscan{kN}.path = settings.path;
    elseif (keyN >= 48) ...
        && (keyN <= 57)
        keyN = str2double(char(keyN));
        refscan{keyN}.colData = data;
        refscan{keyN}.Tag = tg;
    else
        evnt.Character
    end
